...
function App() {
  ...
  const isValidCombination = (numbers) => {
    ...
    const validSum = sum >= 100 && sum <= 170;
    const validOddEven = [2, 3, 4].includes(oddCount);
    const validLowHigh = [2, 3, 4].includes(lowCount);
    const validSequence = !hasTripleSequence;
    return validSum && validOddEven && validLowHigh && validSequence;
  };
  ...
  return (
    <div className="App" style={{ padding: '30px', textAlign: 'center' }}>
      ...
      <p style={{ marginBottom: '10px', color: '#555' }}>안녕 윤재야 😊 오늘도 좋은 번호만 줄게.</p>
      ...
      {numbers.length > 0 && (
        <>
          ...
          <div style={{ marginTop: '12px', color: '#57606f', fontSize: '14px' }}>
            {filterInfo}
          </div>
        </>
      )}
      ...
      {history.length > 0 && (
        <div style={{ marginTop: '36px', textAlign: 'left', maxWidth: '360px', marginInline: 'auto' }}>
